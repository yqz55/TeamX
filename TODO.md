...
## Phase 2 — 终端信息采集（Client 端）

> **目标**: Client 能采集本机硬件/软件/用户/进程/外设信息，并通过 gRPC 上报到 Server。

### 2.1 信息采集模块（`internal/client/collector/`）
- [ ] 硬件信息采集：
  - CPU 型号、核心数
  - 内存总量、使用量
  - 磁盘列表、容量
...
- [ ] 软件信息采集：
  - 操作系统版本、内核版本
  - 已安装软件列表（Linux: dpkg/rpm, Windows: 注册表, MacOS: brew/pkgutil）
- [ ] 用户账号信息采集：
  - 本地用户列表、用户组
  - 当前登录用户
- [ ] 进程信息采集：
  - 进程列表（PID、名称、内存、CPU 占用）
- [ ] 外设信息采集：
  - USB 设备列表
  - 打印机（可选）
...
### 2.2 采集调度
- [ ] 实现定时采集（可配置间隔，默认 60s）
- [ ] 实现事件触发采集（如新进程启动、USB 插入）
- [ ] 采集数据的本地缓存与去重（避免重复上报相同数据）

### 2.3 上报到 Server
- [ ] 实现 gRPC 流式上报（`stream Report`）
- [ ] 上报数据序列化为 Protobuf

### 2.4 验证
- [ ] 分别在 Linux 和 Windows 上测试信息采集
- [ ] 确认 Server 收到完整上报数据
...
### 6.4 内置示例插件
- [ ] `system-info` — 将 Phase 2 的采集逻辑改造为插件
- [ ] `process-monitor` — 进程监控插件（高 CPU/内存告警）
- [ ] `disk-cleaner` — 磁盘清理插件
